...
def enable_directpath(app, dp_arg="1"):
    # app['custom_conf'] = app.get('custom_conf', [])#hzw + ['enable_directpath ' + dp_arg]
...
def go_host(experiment):
    ...
    for cfg in experiment['hosts'][THISHOST]['apps']:
        ...
        print("launcher = {}".format(launcher[system]))
        tm.new_thread(target=launcher[system],args=(cfg, experiment))
    ...
    for i in range(3):  # try a few extra times
        ...
        if len(dates) == 2:
            ...
            if (d1 - d2)**2 == 1:
                return
        time.sleep(1)
    print("ERROR: experiment.py:529 assert False off = {}".format(d1 - d2))
    exit()
    assert False
...
def execute_experiment(experiment):
    ...
    # remotes = ['server129-1']#hzw= list(set(experiment['hosts'].keys()) - set([THISHOST])) ['ubuntu', 'server129-1']
...
if __name__ == '__main__':
    if len(sys.argv) < 2 or sys.argv[1] == "server":
        ...
        for cfg in experiment['hosts'][CLIENT_NAME_201]['apps']:
            launch_shenango_program(cfg, experiment)
            # system = cfg.get('system', experiment.get('system'))
            # print(" launcher = {}".format(launcher[system]))
            tm.new_thread(target=launcher[system],args=(cfg, experiment))
